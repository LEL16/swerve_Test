...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    // m_outtakeSubsystem,
    // () -> -MathUtil.applyDeadband(m_operatorController.getRawAxis(3), 0.01) *
    // m_powerLimit));
    ...
    // NamedCommands.registerCommand("Shoot Note", new InstantCommand(() ->
    // m_outtakeSubsystem.outtakeRotate(0.5)));
    // NamedCommands.registerCommand("Stop Shoot Note", new InstantCommand(() ->
    // m_outtakeSubsystem.outtakeRotate(0.0)));
    // NamedCommands.registerCommand("Intake Note", new InstantCommand(() ->
    // m_intakeSubsystem.intakeRotate(0.5)));
    // NamedCommands.registerCommand("Stop Intake Note", new InstantCommand(() ->
    // m_intakeSubsystem.intakeRotate(0.0)));
  ...
  public Command autonomousCommands() {
    m_powerLimit = 1.0;
    return new PathPlannerAuto("Path 2"); // Debugging return statement
  ...
  private void configureButtons() {
    ...
    // m_pathfinding.onTrue(AutoBuilder.pathfindToPose(new Pose2d(8.30, 4.10,
    // Rotation2d.fromDegrees(90)),
    // new PathConstraints(2.0, 2.0, Units.degreesToRadians(180),
    // Units.degreesToRadians(180)), 0.0, 0.0));
    ...
    // () -> m_operatorController.getRawButton(1));
    // m_pivotLowPosition.whileTrue(new PositionPivotCommand(m_pivotSubsystem,
    // "low"));
    // m_pivotLowPosition.whileFalse(new InstantCommand(() ->
    // m_pivotSubsystem.getCurrentCommand().cancel()));

    // // Operator button B
    // Trigger m_pivotMidPosition = new Trigger(
    // () -> m_operatorController.getRawButton(2));
    // m_pivotMidPosition.whileTrue(new PositionPivotCommand(m_pivotSubsystem,
    // "mid"));
    // m_pivotMidPosition.whileFalse(new InstantCommand(() ->
    // m_pivotSubsystem.getCurrentCommand().cancel()));

    // // Operator button X
    // Trigger m_pivotHighPosition = new Trigger(
    // () -> m_operatorController.getRawButton(4));
    // m_pivotHighPosition.whileTrue(new PositionPivotCommand(m_pivotSubsystem,
    // "high"));
    // m_pivotHighPosition.whileFalse(new InstantCommand(() ->
    // m_pivotSubsystem.getCurrentCommand().cancel()));
  }
